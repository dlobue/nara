...
class IndexerConnection(object):
    ...
    def process(self, document):
        ...
        if document.fields:
            map(self._process_fields(result), document.fields)

        return result

    def _process_fields(self, doc):
        ...
        def wrapper(field):
            try:
                ...
                return
            actions.perform(doc, field.value, context)
    ...
    def add(self, document):
        ...
        try:
            if document.fields and hasattr(document.fields, '__iter__'):
                map(self._process_fields(document), document.fields)
                delattr(document, 'fields')
        except AttributeError:
            #oh well, no new fields.
            pass
